...
def exp2():
    ...
    for epoch in range(training_epochs):
        ...
        for x, y in loader:
            ...
            real_gan_out, real_q_discrete, real_q_mu, real_q_var = discriminator(y)
            loss_d_real = discriminator_loss(real_gan_out, real_labels)

            infogan_input = x

            generated_samples = generator(infogan_input)

            fake_gan_out, fake_q_discrete, fake_q_mu, fake_q_var = discriminator(generated_samples.detach())
            ...
            fake_gan_out, fake_q_discrete, fake_q_mu, fake_q_var = discriminator(generated_samples)
            generator_loss = discriminator_loss(fake_gan_out, real_labels)
